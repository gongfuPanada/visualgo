...
public class GraphUtil {
	...
	private static int minCutIteration(Graph g){
	 ...
	 public static int minCut(String data, String delimiter) throws IOException{
			InputStream is = new ByteArrayInputStream(data.getBytes());
			Graph g = new Graph(DataReader.readGraph(is, delimiter));
			is.close();
			//int nnodes = g.getNodesSize();
			...
			for(int i = 0 ; i < 100; i++){
				...
				g = new Graph(DataReader.readGraph(is, delimiter));
				is.close();
